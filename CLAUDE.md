...
- **Export a figuritas (`core/exporters/figuritas.ts`)**: formato descifrado por ingeniería inversa (DATA_MODEL §10). El encabezado `e2 8b 8b 5e` está hardcodeado de UNA muestra real; si figuritas rechaza el QR, conseguir una 2ª muestra para ver si el encabezado varía. Usa gzip + base64 estándar (¡no deflate-raw/base64url como nuestro codec!). Verificado byte-a-byte contra un export real, pero el import del otro lado solo se confirma escaneando en figuritas.
