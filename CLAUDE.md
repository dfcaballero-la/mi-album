...
Hecho: grilla por secciones con ciclo de estados (falta→tengo→repetida×n, click suma / Ctrl-Cmd+click o mantener presionada resta), filtros todas/solo-repetidas/faltantes, buscador por país (sin tildes, en ambos idiomas) + código + nombre, salto rápido a sección + header sticky, % por sección, stats + estimador de sobres (coupon collector, `core/stats.ts`), codec compacto para QR (`core/codec.ts`), matcher de intercambio 1:1 con priorización (`core/trade-matcher.ts`), pantalla de intercambio con QR real (`qrcode`+`jsqr`, `src/TradeScreen.tsx`), compartir listas de repetidas/faltantes por WhatsApp con banderas emoji (`core/share.ts`, `core/flags.ts`), importador de figuritas.app (`core/importers/figuritas.ts`) y **exportador a figuritas por QR** (`core/exporters/figuritas.ts`, formato descifrado por ingeniería inversa; DATA_MODEL §10; botón en `TradeScreen`), ronda de intercambio multi-parte (`core/trade-circles.ts` + `src/RondaScreen.tsx`, círculos de 3+), export/import de respaldo en UI (con `navigator.share`/AirDrop; importar FUSIONA vía `mergeBackup`), selector de álbum (`src/albums.ts`, oculto si hay uno solo), i18n ES/EN (`core/i18n.ts` + hook `useLocale`, toggle en el header, persiste en Dexie; nombres de país/grupo en `core/section-names.ts`), PWA instalable con íconos generados sin assets externos (`scripts/generate-icons.mjs`), suite e2e con Playwright cubriendo grilla/trueque/respaldo/idioma/ronda/export-figuritas (`tests/e2e/`, corre en CI), **álbum real de 994 láminas (50 secciones, incluye Coca-Cola)**, CI + deploy funcionando.
...
2. **Matching multi-parte (v2.3)** — COMPLETO (motor + UI): `core/trade-circles.ts` `findTradeCircles()` (círculos de 3+; DATA_MODEL §9) y `src/RondaScreen.tsx` (juntar códigos del grupo → círculos rankeados → "confirmar mi parte" aplica a la colección local). Escáner de QR extraído a `src/CodeScanner.tsx` y reutilizado por `TradeScreen`. Todo offline. Diferenciador que figuritas.app no tiene. Falta solo la variante en salas en tiempo real (necesita backend, v2.2).
...
- **El álbum Mundial 2026 tiene 994 láminas (50 secciones), no 980**: incluye la sección `cc` (Coca-Cola, `CC-1`…`CC-14`, índices 980–993) porque el álbum real de figuritas la tiene y el export a figuritas es una foto completa que debe alinear índice-a-índice. Si tocás el conteo, actualizá los e2e que asumen `/994` y el invariante de `section-names`.
- **Export a figuritas (`core/exporters/figuritas.ts`)**: formato descifrado por ingeniería inversa (DATA_MODEL §10). El encabezado `e2 8b 8b 5e` está hardcodeado de UNA muestra real. Usa gzip + base64 estándar (¡no deflate-raw/base64url como nuestro codec!). Verificado byte-a-byte contra un export real y **confirmado en la práctica (2026-09-07): figuritas importa el QR generado sin problemas**. Si en el futuro figuritas cambia su formato y deja de aceptarlo, re-descifrar con una muestra nueva.
